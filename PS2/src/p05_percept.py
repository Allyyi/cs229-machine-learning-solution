...
def initial_state():
    ...
    # *** START CODE HERE ***
    # return an empty tuple to contain beta for each step
    return []
    # *** END CODE HERE ***
...
def predict(state, kernel, x_i):
    ...
    """
    # *** START CODE HERE ***
    return sign(np.sum(beta * kernel(x, x_i) for beta, x in state))
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    """
    # *** START CODE HERE ***
    beta_i = learning_rate * (y_i - sign(np.sum(beta * kernel(x, x_i) for beta, x in state)))
